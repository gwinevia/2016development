...
/**
 * <p>タイトル:キーバインド設定サブダイアログ </p>
 * <p>説明:アクションを実行するキーの組み合わせを作成します </p>
...
public class IdeOptionSub extends EnhancedDialog implements ActionListener {
  ...
  private void jbInit() throws Exception {
    ...
    panel2.add(new JLabel(mainframe.getBilingualMsg("0123") + "："),BorderLayout.WEST);
    ...
    panel4.add(new JLabel(mainframe.getBilingualMsg("0164") + "："), BorderLayout.WEST);
       ...
       for (int j=0; j<vecKeyBind.size(); j++ ) {
         ...
         if (!wkKeyBind.equals(KeyBind) && wkKeyStroke.equals(keystroke2) ) {
           ...
           if (System.getProperty("language").equals("japanese") ) {
             JOptionPane.showOptionDialog(this,keystroke + "は既に、" + wkKeyBind + "にマップされています。","エラー",
