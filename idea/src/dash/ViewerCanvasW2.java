...
public class ViewerCanvasW2 extends ViewerCanvas2 implements ActionListener {
	...
	void contentPane_mouseMoved(MouseEvent e) {
		...
		VIcon vicon = getIconAt(e.getPoint());

		VIconAg viconag = (VIconAg) vicon;
		if (vicon == null) {
			setToolTipText("");
		} else {
			setToolTipText(viconag.getName());
		}
	}
	...
	*/
	...
	public void removeAgentAll() {
		...
		for (Enumeration e = agTable.elements(); e.hasMoreElements();) {
			VIconAg a = (VIconAg) e.nextElement();
			vecAgentName.addElement(a.name);
		}

		for (int i = 0; i < vecAgentName.size(); i++) {
			String agName = (String) vecAgentName.elementAt(i);
			VIconAg ra = (VIconAg) agTable.remove(agName);
