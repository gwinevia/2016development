...
public abstract class DVM implements Runnable {
  ...
  int result = 0;
  int times = 1;
  String sql_st = "SELECT * FROM dashlog_test;";
  ...
  public DVM(String name, File msgfile, boolean useNewif, boolean useViewer, File dashdir) {
    ...
    boolean noGUI = System.getProperty("dash.noGUI", "off").equalsIgnoreCase("on");
  ...
  public void run() {
    while(true) {
      try {
        ...
        // (1)インタフェース宛DashMessage.IF
        if (msg.to.equals("_interface")) {
  ...
  String createAgent(String cname, String description, String filename, String facts, String origin, String oldname, ps.AgentProgram program, ps.WorkMem workmem) {
    ...
    try {
      ...
      DashLog(filename); //Dash　使用エージェントのログ
 ...
 void DashLog(String filename) {
  ...
  void killAgent(String name, DashMessage yuigon) {
    ...
    if (yuigon != null) {
      sendMessageFromUser(yuigon, DashMessage.ACTIVATEMOBILE,"SUCCESS", null, null, "()", null);
  ...
  private byte[] loadLocalClassData(String classname) throws Exception {
    ...
    for (int i=0; i<userClassPath.length; i++) {
      ...
      String filename = userClassPath[i] + classname.replace('.', File.separatorChar) + ".class";
      ...
      filename = userClassPath[i] + classname.replace('.', File.separatorChar) + ".properties";
      classfile = new File(filename);
      if (classfile.exists()) {
        break;
      }
