 ...
 */
ViewerCanvas(String dvmname, boolean isR, Viewer v) {
...
public void showMsg(DashMessage m,  boolean isR) {
  ...
  viewer.writeMsg(m);
    ...
    } else if (m.departure == null || m.departure.equals(dvmname ) )  {
      showAgToAgMsg(m);
    } else {
...
private void showEnvToAgMsg(DashMessage m, boolean isR) {
  ...
  if (m.departure.substring(0,1) == "r" ) {
		isR = true;
	}
	else {
		isR = false;
...
void showUnicastMsg(DashMessage m) {
  VIcon fromIcon = (m.from.equals("_interface")||m.from.equals("KaneHooker") ? getEnvIcon(m.from, isRepository) : (VIconAg)agTable.get(m.from));
  
  VIcon toIcon = (m.to.equals("_interface")||m.to.equals("KaneHooker") ? envIcon : (VIconAg)agTable.get(m.to));
...
public void waitStep() {
  ...
  try {
    ...
    /*AdipsEnv.exit("ViewerCanvas.waitStep()", e); */
  }
}
